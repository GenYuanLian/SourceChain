...
The RPM spec file provided here is for Srcchain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://srcchain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Srcchain git repository](https://github.com/srcchain/srcchain) but are not part of
...
Currently only `Source10` is used, for the example `srcchain.conf` file.
...
in the srcchain git repository. At present only an SVG version of the Srcchain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the srcchain
...
needed when 0.12.1 is released, a proper fix is already in the Srcchain git
master branch.
...
idea to build Srcchain Core against a static version of that library compiled
...
The `srcchain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `srcchain-server` package. At this point, most RPM based
...
The `srcchain-server` package should have SELinux support. How to properly do
...
Srcchain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/srcchain/). He has been
packaging Srcchain for Fedora at least since 2012.
...
4. I renamed the `srcchain` package that contains the Qt GUI to `srcchain-core` as
that appears to be how the general population refers to it, in contrast to
`srcchain-xt` or `srcchain-classic`. I wanted to make sure the general population
