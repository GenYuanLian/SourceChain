...
###   srcchaincore.org and srcchain.org and compares them.
...
WORKINGDIR="/tmp/srcchain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://srcchaincore.org"
HOST2="https://srcchain.org"
BASEDIR="/bin/"
VERSIONPREFIX="srcchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'srcchain-',
...
if [ $? -ne 0 ]; then
   echo "srcchain.org failed to provide signature file, but srcchaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "srcchain.org and srcchaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Srcchain Core binary release signing key installed?"
   fi
