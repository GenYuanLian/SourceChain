...
*Setup instructions for a Gitian build of Srcchain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Srcchain
...
- [Building Srcchain Core](#building-srcchain-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-srcchain-host/README.md).
...
Clone the git repositories for srcchain and Gitian.
...
in the srcchain repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Srcchain Core
----------------

To build Srcchain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the srcchain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/srcchain/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/srcchain/srcchain
...
For example:
```bash
URL=https://github.com/laanwj/srcchain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit srcchain=${COMMIT} --url srcchain=${URL} ../srcchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit srcchain=${COMMIT} --url srcchain=${URL} ../srcchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit srcchain=${COMMIT} --url srcchain=${URL} ../srcchain/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the srcchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../srcchain/contrib/gitian-descriptors/*|sort|uniq )
...
STKPATH=/some/root/path/srcchain
SIGPATH=/some/root/path/srcchain-detached-sigs

./bin/gbuild --url srcchain=${STKPATH},signature=${SIGPATH} ../srcchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/srcchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/srcchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/srcchain-osx-build.assert
```
...
[srcchain-core/gitian.sigs](https://github.com/srcchain-core/gitian.sigs/) repository, or if that's not possible create a pull
