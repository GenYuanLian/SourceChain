...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s srcchain-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  srcchain-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded srcchain transaction") + "\n" +
              "  srcchain-tx [options] -create [commands]   " + _("Create hex-encoded srcchain transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        ...
        CSrcchainAddress redeemScriptAddr(scriptPubKey);
...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        // Get the address for the redeem script, then call
        // GetScriptForDestination() to construct a P2SH scriptPubKey.
        CSrcchainAddress addr(scriptPubKey);
        ...
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                if (prevOut.exists("amount")) {
                    srcchain.out.nValue = AmountFromValue(prevOut["amount"]);
                }
                srcchain.nHeight = 1;
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded srcchain transaction
