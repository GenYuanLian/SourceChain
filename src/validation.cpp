...
#if defined(NDEBUG)
# error "Srcchain cannot be compiled without assertions."
#endif
...
bool fReindex = false;
bool fTxIndex = false;
bool fAddressIndex = false;  //added for bitpay
bool fTimestampIndex = false;//added for bitpay
bool fSpentIndex = false;    //added for bitpay
...
const std::string strMessageMagic = "Srcchain Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // Add memory address index //added for bitpay
        if (fAddressIndex) {
            pool.addAddressIndex(entry, view);
        }

        // Add memory spent index //added for bitpay
        if (fSpentIndex) {
...
void ThreadScriptCheck() {
    RenameThread("srcchain-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    {
        const CTransaction &tx = *(block.vtx[i]);
        const uint256 txhash = tx.GetHash(); //added for bitpay
        nInputs += tx.vin.size();
