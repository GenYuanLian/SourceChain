...
std::string WalletUtilityDB::getAddress(CDataStream ssKey)
{
    ...
    std::string strAddr = CSrcchainAddress(id).ToString();
...
std::string WalletUtilityDB::getKey(CDataStream ssKey, CDataStream ssValue)
{
    ...
    if (key.Load(pkey, vchPubKey, true))
        strKey = CSrcchainSecret(key).ToString();
...
std::string WalletUtilityDB::getCryptedKey(CDataStream ssKey, CDataStream ssValue, std::string masterPass)
{
    ...
    std::string strKey = CSrcchainSecret(key).ToString();
    return strKey;
}
...
int main(int argc, char* argv[])
{
    ...
    std::string walletFile = Args.GetArg("-wallet", "wallet.dat");
    std::string masterPass = Args.GetArg("-pass", "");
    bool fDumpPass = Args.GetBoolArg("-dumppass", false);
    bool help = Args.GetBoolArg("-h", false);
    bool result = false;
