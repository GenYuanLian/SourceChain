// Copyright (c) 2015-2018 genyuanlian
...
#include "crypto/schash/schash.h"
#include "crypto/schash/Lyra2.h"
...
//    LogPrintf("%s\n", pblock->ToString());
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SrcchainMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(chainparams, shared_pblock, true, nullptr))
        return error("SrcchainMiner: ProcessNewBlock, block not accepted");
...
void GenerateSrcchains(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SrcchainMiner, boost::cref(chainparams)));
}
