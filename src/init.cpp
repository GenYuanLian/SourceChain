 ...
 * between srcchaind, and srcchain-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SRCCHAIN_CONF_FILENAME));
    if (mode == HMM_SRCCHAIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SRCCHAIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/GenYuanLian/SourceChain>";
    const std::string URL_WEBSITE = "<https://www.genyuanlian.com/>";
...
/** Sanity checks
 *  Ensure that Srcchain is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Srcchain process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SRCCHAIN_CONF_FILENAME)).string());
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                pblocktree = new CBlockTreeDB(nBlockTreeDBCache, false, fReindex, dbCompression, dbMaxOpenFiles); //modified for bitpay
                ...
                //将创世纪块的交易加载交易缓存mempool中, 使相关交易能消费
                {
    ...
    GenerateSrcchains(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS),
                     chainparams);
